# Script to setup MySQL database for Core API
# This script creates the database and runs migrations

Write-Host "🚀 Setting up MySQL Database for Core API" -ForegroundColor Green
...
try {
    ...
    & $mysqlCmd @connectionArgs -e "CREATE DATABASE IF NOT EXISTS core_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Database 'core_db' created successfully!" -ForegroundColor Green
    } else {
        throw "Failed to create database"
    }
    
    # Test connection to the new database
    & $mysqlCmd @connectionArgs -e "USE core_db; SELECT 'Connection successful!' as status;"
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Connection to core_db successful!" -ForegroundColor Green
    } else {
        throw "Failed to connect to core_db"
    }
...
if ($rootPassword) {
    $newConnectionString = "mysql://root:$rootPassword@localhost:3306/core_db"
} else {
    $newConnectionString = "mysql://root:@localhost:3306/core_db"
}
...
try {
    ...
    Write-Host "2. Verify database 'core_db' exists" -ForegroundColor White
...
try {
    & $mysqlCmd @connectionArgs -e "USE core_db; SHOW TABLES;"
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "\n✅ MySQL Database Setup Complete!" -ForegroundColor Green
        Write-Host "\n📊 Database Information:" -ForegroundColor Cyan
        Write-Host "- Database: core_db" -ForegroundColor White
        ...
        Write-Host "- Sample Data: Seeded (users, leads, appointments, etc.)" -ForegroundColor White
        
        Write-Host "\n🚀 Next Steps:" -ForegroundColor Green
        Write-Host "1. Start the API server: npm run dev" -ForegroundColor White
        Write-Host "2. Test API: http://localhost:3001/api/core/users" -ForegroundColor White
        Write-Host "3. View Swagger docs: http://localhost:3001/api/docs" -ForegroundColor White
...
Write-Host "\n📚 For more help, see readme.txt" -ForegroundColor Cyan